...
RETINA_HOST = os.getenv("RETINA_HOST")
RETINA_PORT = os.getenv("RETINA_PORT")
ARCFACE_HOST = os.getenv("ARCFACE_HOST")
ARCFACE_PORT = os.getenv("ARCFACE_PORT")
...
class InformationModel(object):
    def __init__(self):
        ...
        self.face_api = f"http://{RETINA_HOST}:{RETINA_PORT}/predict/retina"
        self.arc_api = f"http://{ARCFACE_HOST}:{ARCFACE_PORT}/predict/arc_face"
...
def detect_face(image_queue, detections_queue, info_model, info_video):
    ...
    while info_video.cap.isOpened():
        ...
        img = scale_image(image_rgb, scale_percent=scale)
        # img = image_rgb
        _start_time = time.time()
        files = [("image", nparray_to_bytebuffer(img))]
        payload = {"ori_height": img.shape[0], "ori_width": img.shape[1], "detection_scale": scale}
        response = requests.request("POST", info_model.face_api, files=files, data=payload)
        response = response.json()
        face_boxes, landmarks = np.array(response["boxes"]), np.array(response["points"])
        print("rerina detect_face", time.time() - _start_time)

        detections_queue.put([face_boxes, landmarks, image_rgb, index_frame])
    info_video.cap.release()
    ...
    while info_video.cap.isOpened():
        ...
        if len(face_boxes) > 0:
            ...
            for i in range(len(face_boxes)):
                ...
                files = [("image", nparray_to_bytebuffer(aligned_face))]
                response = requests.request("POST", info_model.arc_api, files=files)
                response = response.json()
                encoding_vector = np.array(response["encoding_vector"])
